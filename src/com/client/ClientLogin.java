	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (captchaDB.equals(vercode)) {
				ResultSet resultset = st.executeQuery("select * from tblclient where uname='" + uname + "' AND upass='" + pass + "'");
				if (resultset.next()) {
					hs.setAttribute("uname",resultset.getString("uname") );
					hs.setAttribute("client_name", resultset.getString("client_name"));
					hs.setAttribute("client_mobile", resultset.getString("mobile_no"));
					int update = DatabaseConnection.insertUpdateFromSqlQuery("update tblcaptcha set captcha='"+ newRandomCaptcha + "'");
					response.sendRedirect("client-dashboard.jsp?_tokens='" + tokens + "'");
				} else {

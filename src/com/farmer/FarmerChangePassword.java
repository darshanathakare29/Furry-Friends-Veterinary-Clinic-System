...
public class FarmerChangePassword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet resultset = statement.executeQuery("select upass from tblfarmer where upass='" + cpassword+ "' and uname='" + session.getAttribute("uname") + "'");
			if (resultset.next()) {
				pass = resultset.getString(1);
			}
			if (password.equals(confpass)) {
				if (pass.equals(cpassword)) {
					int i = statement.executeUpdate("update tblfarmer set upass='" + password + "' where uname='" + session.getAttribute("uname") + "' ");
